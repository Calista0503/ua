  ...
  <div>
    <div class="toolbar">
      <el-form :inline="true" :model="q" size="mini">
        ...
        <Merchant @getOptions="getOptions" title="所属商户" multiple v-model="q.mch_ids"></Merchant>
    ...
    >
      ...
      <el-table-column prop="mch_name" fixed="left" label="所属商户"></el-table-column>
...
export default {
  ...
  methods: {
    asyncRecord() {
      this.$refs.recordDialog.open('MCH_REPORT');
    },
    downloadData() {
      this.$vcode().then(() => {
        this.downloading = true;
        Object.assign(this.q, {export: 1})
        mchApi.getReportList(this.$qfilter(this.q)).then(res => {
          if (res.status===0){
            this.$message({
              type: 'success',
